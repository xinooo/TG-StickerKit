...
- **`scripts/`**: 存放所有核心 Python 程式碼。
  ...
  - **`utils/`**: 共用工具
    ...
    - `tg_utils.py`: Telegram 貼圖包重複偵測工具。
...
3. **自動化執行**: 
   ...
   - 上傳前自動檢查 Telegram 是否已有相同貼圖包，避免重複創建。
   - 自動完成轉檔與上傳。
...
- **重複偵測**: 執行一鍵流程時，腳本會先檢查 Telegram 上是否已有相同 short_name 的貼圖包，若存在則自動中止以節省資源。若想強制重新建立，請修改 `config.json` 中的 `pack_short_name`。
