...
from telethon.tl.types import InputStickerSetItem, InputMediaUploadedDocument, DocumentAttributeFilename
...
if os.path.exists(CONFIG_PATH):
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        config_data = json.load(f).get('static', {})
...
# 貼圖包設定
PACK_TITLE = config_data.get('pack_title') or 'My Custom Stickers'
PACK_SHORT_NAME = config_data.get('pack_short_name') or 'my_custom_pack'
DEFAULT_EMOJI = config_data.get('default_emoji') or '✨'

# 資料夾路徑
RELATIVE_DIR = config_data.get('sticker_dir') or 'static/output'
STICKER_DIR = os.path.join(WORKSPACE_DIR, RELATIVE_DIR.replace('static/', '')) if RELATIVE_DIR.startswith('static/') else os.path.join(WORKSPACE_DIR, "static", "output")
if not RELATIVE_DIR.startswith('static/'):
...
async def main():
    ...
    files = [f for f in os.listdir(STICKER_DIR) if f.lower().endswith('.png')]
    if not files:
        print(f"資料夾 {STICKER_DIR} 內沒有可上傳的 PNG 檔案。")
        return

    print(f"準備上傳 {len(files)} 張貼圖...")
    ...
    try:
        ...
        print(f"正在為使用者 {me.first_name} 建立貼圖包...")
